...
//  MotionMeCrazy
//
//  Created by Elasia Rodriguez on 4/1/25.
//
...
final class MoveNet: PoseEstimator {
  ...
  init(threadCount: Int, delegate: Delegates, modelType: ModelType) throws {
    ...
    guard
      let modelPath = Bundle.main.path(
        forResource: "movenet_lightning",
        ofType: "tflite")
    else {
      fatalError("Failed to load the model file with name: 'movenet_lightning.tflite'.")
    }
  ...
  func estimateSinglePose(on pixelBuffer: CVPixelBuffer) throws -> Person {
